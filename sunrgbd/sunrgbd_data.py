...
DEFAULT_TYPE_WHITELIST = ['cabinet','bed','chair','sofa','table','door','window','bookshelf','picture','counter','blinds',
                          'desk','shelves','curtain','dresser','pillow','mirror','floor_mat','clothes','books','fridge',
                          'paper','towel','shower_curtain','box','whiteboard','night_stand','toilet','sink','lamp','bathtub','bag']
...
def extract_sunrgbd_data(idx_filename, split, output_folder, num_point=20000,
    ...
    for data_idx in data_idx_list:
        ...
        if skip_empty_scene and (len(objects)==0):
                continue
        ...
        for obj in objects:
            ...
            bbox2d_list.append(obj.box2d)
            name_list.append(obj.classname)
        if len(object_list)==0:
            obbs = np.zeros((0,8))
            bboxs2d=np.zeros((0,4))
            name_list=[]
        else:
            obbs = np.vstack(object_list) # (K,8)
            bboxs2d=np.vstack(bbox2d_list)

        with open(os.path.join(output_folder,"%06d_namelist.txt"%(data_idx)),'w') as f:
        ...
        np.save(os.path.join(output_folder, "%06d_bbox2d.npy"%(data_idx)),bboxs2d)
...
if __name__=='__main__':
    ...
    if args.gen_v1_data:
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
            ...
            save_votes=True, num_point=30000, use_v1=True, skip_empty_scene=False)
    ...
    if args.gen_v2_data:
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_30k_v2'),
            save_votes=True, num_point=30000, use_v1=False, skip_empty_scene=False)
        extract_sunrgbd_data(os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_30k_v2'),
            save_votes=True, num_point=30000, use_v1=False, skip_empty_scene=False)
