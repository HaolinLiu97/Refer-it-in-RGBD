...
class RGBD_RefNet(nn.Module):
    def __init__(self, cfg,is_train=True,max_nseed=256, pcd_in_channels=3, pcd_hidden_size=256, lang_hidden_size=256,
                  , alpha=1, t=1):
        super().__init__()
        self.is_train=is_train
        self.NN_atten=NN_atten
        self.alpha=alpha
        self.t=t
        self.is_atten_trainable = is_atten_trainable
        self.max_nseed = max_nseed
        self.encoder = modules.keypoint_encoder(in_channels=pcd_in_channels, output_channels=pcd_hidden_size,
        ...
        self.fuse_text_seed = nn.Sequential(
            nn.Conv1d(in_channels=pcd_hidden_size + lang_hidden_size, out_channels=512, kernel_size=1),
            nn.ReLU(),
    ...
    def forward(self, input_dict):
        ...
        pcd_heatmap = input_dict["pcd_heatmap"].squeeze(1)
